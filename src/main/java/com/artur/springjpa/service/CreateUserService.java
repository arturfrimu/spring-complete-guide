...
public class CreateUserService implements CreateUserUseCase {
    ...
    private static void validate(CreateUserCommand command) {
        if (isNull(command.phoneNumber()) || command.phoneNumber().isBlank()) {
            throw new IllegalArgumentException(String.format("Phone number can not be blank %s", command.phoneNumber()));
        }

        if (isNull(command.username()) || command.username().isBlank()) {
            throw new IllegalArgumentException(String.format("Username can not be blank %s", command.username()));
        }

        if (isNull(command.password()) || command.password().isBlank()) {
            throw new IllegalArgumentException(String.format("Password can not be blank %s", command.password()));
        }
    }
